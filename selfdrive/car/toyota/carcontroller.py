...
class CarController():
  ...
  def update(self, enabled, active, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    if not enabled or CS.steer_state in [9, 25] or abs(CS.out.steeringRateDeg) > 100 or abs(CS.out.steeringAngleDeg) > 400 or (abs(CS.out.steeringAngleDeg) > 150 and CS.CP.carFingerprint in [CAR.RAV4H, CAR.PRIUS]):
      apply_steer = 0
      apply_steer_req = 0
    else:
      apply_steer_req = 1
